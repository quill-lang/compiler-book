...
Notably, we do not specify specific algorithms for such tasks as visibility checking, module systems, dependency management, type inference and borrow checking, since these do not contribute significantly to proving the correctness of a program at _run time_. It is therefore left as an implementation detail to provide a correct type inference algorithm and borrow checker, although proofs for certain cases may be supplied here eventually.
